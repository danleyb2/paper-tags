...
class PaperTagsInput extends PaperTagsMixin(PolymerElement) {
  static get template() {
    ...
    <paper-input id="tagInput" always-float-label="" readonly\$="[[readonly]]" placeholder\$="[[placeholder]]" disabled\$="[[disabled]]" label="[[label]]" value="{{value}}" on-tap="_onInputTap" maxlength\$="[[maxlength]]" error-message="[[errorMessage]]" minlength\$="[[minlength]]" invalid="{{invalid}}">
      <slot name="prefix" slot="prefix"></slot>
      <paper-tags id="paperTags" readonly\$="[[readonly]]" items="{{items}}" icon="[[icon]]" get-style="[[getStyle]]" get-name="[[getName]]" get-label="[[getLabel]]" item-class="[[itemClass]]" class-path="[[classPath]]" label-path="[[labelPath]]" icon-path="[[iconPath]]" prevent-remove-tag="[[preventRemoveTag]]" slot="prefix"></paper-tags>
